...
import {downloadCsv} from '../utils/csv.util.js';
...
const getReport = async (req, res) => {
    try {
        ...
        const report = await orderRepairsService.getReport(req.query, true);

        const response = report.length > 0 ? report[0] : {totalAmount: 0, count: 0};
...
const getReportInCsv = async (req, res) => {
    try {

        const report = await orderRepairsService.getReport(req.query);

        const fields = [
            {label: 'No. orden', value: 'orderId'},
            {label: 'Fecha', value: 'createdAt'},
            {label: 'Nombre', value: 'customer.name'},
            {label: 'Apellido', value: 'customer.lastName'},
            {label: 'Segundo apellido', value: 'customer.surName'},
            {label: 'Telefono', value: 'customer.phone'},
            {label: 'Total', value: 'totalAmount'},
        ];

        const data = report.map(order => {
            const createdAt = new Date(order.createdAt);
            const month = `${createdAt.getMonth() + 1}`;
            order.createdAt = `${createdAt.getDate()}-${month.padStart(2, '0')}-${createdAt.getFullYear()}`;
            return order;
        });

        downloadCsv(res, 'ordenes', fields, data);
